...
const QUOTE_HEADERS_REGEX = [
    /^\s*(On\s[\S\s]+?wrote:)$/m, // On DATE, NAME <EMAIL> wrote:
    /^\s*(Le\s[\S\s]+?écrit :)$/m, // Le DATE, NAME <EMAIL> a écrit :
    /^\s*(El\s[\S\s]+?escribió:)$/m, // El DATE, NAME <EMAIL> escribió:
    /^\s*(Il\s[\S\s]+?scritto:)$/m, // Il DATE, NAME <EMAIL> ha scritto:
    /^\s*(Op\s[\S\s]+?schreef.+:)$/m, // Il DATE, schreef NAME <EMAIL>:
    ...
    /^\s*(From\s?:[\S\s]+\s?([|<).+(]|>))/u, // "From: NAME <EMAIL>" OR "From : NAME <EMAIL>" OR "From : NAME<EMAIL>"(With support whitespace before start and before <)
    /^\s*(De\s?:[\S\s]+\s?([|<).+(]|>))/u, // "De: NAME <EMAIL>" OR "De : NAME <EMAIL>" OR "De : NAME<EMAIL>"  (With support whitespace before start and before <)
    /^\s*(Van\s?:[\S\s]+\s?([|<).+(]|>))/u, // "Van: NAME <EMAIL>" OR "Van : NAME <EMAIL>" OR "Van : NAME<EMAIL>"  (With support whitespace before start and before <)
    /^\s*(Da\s?:[\S\s]+\s?([|<).+(]|>))/u, // "Da: NAME <EMAIL>" OR "Da : NAME <EMAIL>" OR "Da : NAME<EMAIL>"  (With support whitespace before start and before <)
...
class Parser {
     ...
     * @returns {Email} the parsed email, or null if not parseable
     */
    parse(text) {
        ...
        return new Email(emailFragments);
    }
    ...
    _isFragmentLine(fragment, line, isQuoted) {
        ...
        if (fragment.isQuoted) {
            return this._isQuoteHeader(line) && fragment.lines.length === 0;
        }
     ...
     */
    _isSignature(text) {
        return this._signatureRegex.test(text);
    }
     ...
     */
    _isQuoteHeader(text) {
        return _.filter(this._quoteHeadersRegex, exp => exp.test(text)).length > 0;
    }
}
